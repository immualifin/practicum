...
class DetailScreen extends StatelessWidget {
  ...
  Future<Surah> _getDetailSurah() async {
    String data =
        await rootBundle.loadString('assets/datas/surah/$noSurat.json');
    Map<String, dynamic> surahData = json.decode(data);

    //
    Map<String, dynamic> surah = surahData['$noSurat'];
    //

    // get list surah
    String dataList =
        await rootBundle.loadString('assets/datas/list-surah.json');
    List<dynamic> listSurah = json.decode(dataList);
    Map<String, dynamic> surahInfo =
        listSurah.firstWhere((element) => element['nomor'] == noSurat);

    List<Ayat> ayat = [];
    for (var i = 0; i < surahInfo['jumlah_ayat']; i++) {
      ayat.add(Ayat(
          id: i,
          surah: noSurat,
          nomor: i + 1,
          ar: surah['text']['${i + 1}'],
          tr: '',
          idn: surah['translations']['id']['text']['${i + 1}']));
    }

    return Surah(
        nomor: surahInfo['nomor'],
        nama: surahInfo['nama'],
        namaLatin: surahInfo['nama_latin'],
        jumlahAyat: surahInfo['jumlah_ayat'],
        tempatTurun: surahInfo['tempat_turun'] == 'mekah'
            ? TempatTurun.MEKAH
            : TempatTurun.MADINAH,
        arti: surahInfo['arti'],
        deskripsi: surahInfo['deskripsi'],
        audio: surahInfo['audio'],
        ayat: ayat);
  }
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              const Spacer(),
              Icon(Icons.share_outlined, color: cardColor),
              const SizedBox(width: 16),
              Icon(Icons.play_arrow_outlined, color: cardColor),
              const SizedBox(width: 16),
              Icon(Icons.bookmark_outline, color: cardColor),
            ],
          ),
        ),
